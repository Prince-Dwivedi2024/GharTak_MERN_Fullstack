...
const Appointment = () => {
  ...
  for(let i =0; i<7; i++){
    ...
    while(currentDate < endTime){
     ...
     const isSlotAvailable = workInfo.slots_booked[slotDate] && workInfo.slots_booked[slotDate].includes(slotTime) ? false : true
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      const date = workSlots[slotIndex][0].datetime
      ...
      const {data} = await axios.post(backendUrl + '/api/user/book-appointment', {workId, slotDate, slotTime}, {headers: {Authorization: `Bearer ${token}`,},})
      if(data.success){
